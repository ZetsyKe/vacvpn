...
COPY config.json /usr/local/etc/xray/config.jsonFROM python:3.11-slim
...
# Сначала запускаем Xray, потом FastAPI
CMD ["sh", "-c", "/usr/local/bin/xray run -config /usr/local/etc/xray/config.json & sleep 2 && uvicorn main:app --host 0.0.0.0 --port $PORT"]
